...
- Bundle: `@vercel/go` globs `includeFiles` from `api/` (`../` is illegal). `web/`, `locales/`, and `migrations/` live under `api/` so `{web,locales,migrations}/**` packs them. `platform.AssetDir` walks cwd/parents and `api/<rel>`, so local `go run ./cmd/web` still uses repo-root names.
