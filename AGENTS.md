...
- Players edit their own characters. Campaign DM may edit HP, temp HP, death saves, and apply/remove statuses; other sheet mutations stay owner-only. Fellow players cannot edit. **Players cannot add statuses.** Owner may remove non-hidden statuses; hidden (curse) rows are DM-only to see and remove.
...
- Damage at 0 current HP adds **+1 death fail**. Three fails = dead. Healing stands the character up unless already dead (heal after death is a no-op).

## Battle

- Dedicated `/campaigns/{id}/battle`. Campaign show: **Start battle** (DM) or **Open battle** if a row exists. **One encounter per campaign** until the DM dismisses. Loot is out of scope.
- Wizard: add catalog monsters (qty → one unit row each; debounce 300ms, do not remount search). Setup list is grouped; DM **Edit** sets HP/AC/six ability scores and resist/immune/vuln on every copy of that type still in setup (catalog defaults; stubs start at 1 HP / scores 10). Edit modal (and DM fight-board stats) link the snapshotted 5e14 bestiary article (`source_url_ru`, else `source_url`; hide if empty; new tab). Snapshot lives on `battle_units` (score columns + `resist_json` + `source_url_ru`); later adds of that type inherit it. Typed monster damage uses that snapshot (resist/immune/vuln, same as PC grants). Then initiative (IRL or `1d20+DEX`). All campaign PCs auto-added. Sort initiative desc, DEX tie-break. Round starts at 1. **Next unit** skips dead and escaped; wrap increments round. Becoming active ticks damaging statuses; leaving the turn decrements `duration_turns`.
...
- Campaign SSE event `battle` refreshes `#battle-board`. PC sheet vitals keep per-character SSE.

## Data

- Migrations `001`–`017`. `data/` is not in git. Do not commit `.cursor/mcp.json`.
