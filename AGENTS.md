...
- Bundle: `@vercel/go` globs `includeFiles` from `api/` (`../` is illegal). Assets are **`//go:embed`** from the module root (`bundled.go`) and unpacked to `ASSET_ROOT` when `VERCEL=1`. Local `go run ./cmd/web` still uses repo-root relative paths. `platform.AssetDir` walks cwd/parents and also `api/<rel>`.
